...
def _build_result_dict(df_submit):
    ...
    return {
        str(cust_id): [str(item).strip() for item in list(items)[:10]]
...
def main():
    try:
        ...
        if df_final is None:
            ...
            save_path_s1 = f"candidates_inference_{cache_tag}.parquet"
            ...
            def load_or_generate_candidates(split_name, q_hist, target_customers=None):
                save_path = f"candidates_{split_name}_{cache_tag}.parquet"
